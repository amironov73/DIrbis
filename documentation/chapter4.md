...
Файл меню. Состоит из записей `MenuEntry`, каждая из которых содержит пару строк:

| Поле           | Назначение 
|----------------|-----------
| string code    | Кодовое значение.
| string comment | Комментарий (может быть пустым).

`MenuFile` содержит только одно поле: `MenuEntry[] entries` -- слайс записей. Предоставляет методы:
...
**bool addFormat(string text)** -- добавление в запрос спецификации формата. Если спецификация не приводит к форматированию (например, формат -- пустая строка), возвращается `false`. При необходимости автоматически выбирается кодировка UTF-8. Вместе со спецификацией формата добавляется перевод строки.
